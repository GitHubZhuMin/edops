...
- **不要引入新的核心配置机制。** 所有模块的启用和配置都应通过 `EDOPS_ENABLED_MODULES` 和现有的 `config.yml` 系统进行管理。避免创建新的、并行的配置系统。
- **配置预设应保持原子性。** 在 `tutor/templates/config/presets/` 中定义的预设应只包含必要的 `EDOPS_ENABLED_MODULES` 列表，避免在预设中包含环境特定的敏感信息（如 IP 或密码）。
